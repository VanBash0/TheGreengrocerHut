...
void ClientsGenerator::InitializeClients()
{
    ...
    {
        UE_LOG(LogTemp, Warning, TEXT("[ClientsGenerator] ƒень %d: доступно только %d уникальных BodyPart, а MinSymptoms=%d. Ѕаланс новых симптомов не поспевает за сложностью."), gameMetrics.DayNumber, maxAvailableSymptoms, gameSettings->MinSymptoms);
    }
#endif
}
...
void ClientsGenerator::FillSymptoms()
{
    ...
    // а GenerateDemonSymptoms мог реально набрать меньше Ч синхронизируем.
    ...
    for (int i = 0; i < daySnapshot.DayClients.Num(); ++i)
    {
        ...
        {
            // ќбрезаем массив демона под реально собранный список DemonSymptoms Ч
...
void ClientsGenerator::EnsureSymptomsDontMatchDemon(FClient& client)
{
    ...
    {
        return; // разное количество симптомов Ч совпасть как множество не может
    }
    ...
    {
        return; // не совпадает Ч всЄ ок
    }

    // ѕолное совпадение Ч подмен€ем один случайный симптом клиента на что-то другое.
    TSet<EBodyPart> occupiedParts;
    for (const FName& Symptom : client.Symptoms)
    {
        if (const FSymptomWithWeightsData* Data = gameMetrics.SymptomMetrics.Find(Symptom))
