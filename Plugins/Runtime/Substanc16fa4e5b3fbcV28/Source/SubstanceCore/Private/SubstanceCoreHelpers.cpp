...
//	FTexturePlatformData* Texture = *TextureOutput->GetRunningPlatformData();
	...
	UE_LOG(LogTemp, Error, TEXT("[SubstanceDebug] UpdateSubstanceOutput START: Texture=%p Name='%s' SubstancePixelFormat=%d ChannelsOrder=%d -> UEPixelFormat=%d mipmapCount=%d level0=%dx%d bufferPtr=%p"),
		TextureOutput, *TextureOutput->GetName(),
		(int32)ResultText.pixelFormat, (int32)ResultText.channelsOrder, (int32)Texture->PixelFormat,
		ResultText.mipmapCount, ResultText.level0Width, ResultText.level0Height, ResultText.buffer);

	const bool bDownsampleFrom16Bit = (Texture->PixelFormat == PF_R16G16B16A16_UINT);
	...
	UE_LOG(LogTemp, Error, TEXT("[SubstanceDebug] UpdateSubstanceOutput after resize-check: Texture=%p Mips.Num()=%d SizeX=%d SizeY=%d PixelFormat=%d"),
		TextureOutput, Texture->Mips.Num(), Texture->SizeX, Texture->SizeY, (int32)Texture->PixelFormat);
	...
	UE_LOG(LogTemp, Error, TEXT("[SubstanceDebug] UpdateSubstanceOutput END: Texture=%p FinalMips.Num()=%d"),
		TextureOutput, Texture->Mips.Num());
}
...
void UpdateTexture(const SubstanceTexture& result, SubstanceAir::OutputInstance* Output, bool bCacheResults /*= true*/)
{
	...
	UE_LOG(LogTemp, Error, TEXT("[SubstanceDebug] UpdateTexture: Output='%s' TextureOutput=%p Name='%s' Path='%s' bCacheResults=%s"),
		*FString(Output->mDesc.mIdentifier.c_str()),
		TextureOutput,
		TextureOutput ? *TextureOutput->GetName() : TEXT("<null>"),
		TextureOutput ? *TextureOutput->GetPathName() : TEXT("<null>"),
		bCacheResults ? TEXT("true") : TEXT("false"));

	if (!TextureOutput)
	{
		UE_LOG(LogTemp, Error, TEXT("[SubstanceDebug] UpdateTexture: TextureOutput is NULL, aborting for output '%s'"), *FString(Output->mDesc.mIdentifier.c_str()));
		return;
	}
	...
	UE_LOG(LogTemp, Error, TEXT("[SubstanceDebug] UpdateTexture DONE: Output='%s' TextureOutput=%p"),
		*FString(Output->mDesc.mIdentifier.c_str()), TextureOutput);
}
...
void Tick()
{
	...
	for (const auto& OutputItr : Outputs)
	{
		if (OutputItr->mDesc.isImage())
		{
			...
			if (resultImage)
			{
				UpdateTexture(resultImage->getTexture(), OutputItr, WITH_EDITOR != 0);
#if WITH_EDITOR	
...
bool UpdateTextures(SubstanceAir::shared_ptr<SubstanceAir::GraphInstance> Instance, bool InEditor)
{
	...
	for (const auto& OutputItr : Instance->getOutputs())
	{
		...
		if (OutputItr->mDesc.isImage())
		{
			...
			UE_LOG(LogTemp, Error, TEXT("[SubstanceDebug] Output '%s' resultImage valid=%s"), *FString(OutputItr->mDesc.mIdentifier.c_str()), resultImage ? TEXT("true") : TEXT("false"));
